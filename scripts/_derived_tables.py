...
def require_tables(conn: sqlite3.Connection, tables: tuple[str, ...]) -> None:
    missing = []
    for t in tables:
        row = conn.execute(
...
def carry_expand(
    ...
    指标逐日前向携带，age = 交易日与公告日的自然日差。
    """
    cal = pd.read_sql_query(CAL_SQL, conn)
    ...
    for i in range(len(events)):
        ...
        data[age_col] = cal_int[start:end] - int(ann_int[i])
...
def atomic_replace(
    ...
    try:
        df.to_sql(tmp_table, conn, if_exists="append", index=False, method="multi", chunksize=chunksize)
    except Exception:
        cur.execute(f"DROP TABLE IF EXISTS {tmp_table}")
        conn.commit()
        raise
...
def final_span(conn: sqlite3.Connection, final_table: str) -> tuple[int, str | None, str | None]:
    return conn.execute(
